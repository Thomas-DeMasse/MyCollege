...
onAuthStateChanged(auth, (user) => {
    ...
    SignInWithGoogleButton.removeEventListener("click", userSignOut);
    SignInWithGoogleButton.removeEventListener("click", userSignIn);

    if (user) {
        loginlogoutreference.innerHTML = "Logout";
        loginlogoutreference.addEventListener("click", userSignOut);

        SignInWithGoogleButton.innerHTML = '<i class="fab fa-google google-icon"></i> Sign out';
        SignInWithGoogleButton.addEventListener("click", userSignOut);
        ...
        SignInWithGoogleButton.innerHTML = '<i class="fab fa-google google-icon"></i> Sign in with Google';
        SignInWithGoogleButton.addEventListener("click", userSignIn);

        // TODO() toast saying the user that they are not login and can not save data
